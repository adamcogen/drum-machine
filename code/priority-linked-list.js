...
function _testInsertNodeAtIndex() {
    ...
    Util.assertEquals(0, list.getSize(), "assert empty list size is 0")
    let firstInsertedNode = new PriorityLinkedListNode("3", 3)
    list._insertNodeAtIndex(firstInsertedNode, 0)
    Util.assertEquals(1, list.getSize(), "assert list size is as expected")
    let secondInsertedNode = new PriorityLinkedListNode("5", 5)
    list._insertNodeAtIndex(secondInsertedNode, 1)
    Util.assertEquals(2, list.getSize(), "assert list size is as expected")
    Util.assertEquals(0, list._getIndexOfPriority(2), "check index of priority on small list")
    Util.assertEquals(0, list._getIndexOfPriority(3), "check index of priority on small list")
    Util.assertEquals(1, list._getIndexOfPriority(4), "check index of priority on small list")
    let thirdInsertedNode = new PriorityLinkedListNode("1", 1)
    list._insertNodeAtIndex(thirdInsertedNode, 0)
    Util.assertEquals(3, list.getSize(), "assert list size is as expected")
    let fourthInsertedNode = new PriorityLinkedListNode("2", 2)
    list._insertNodeAtIndex(fourthInsertedNode, 1)
    Util.assertEquals(4, list.getSize(), "assert list size is as expected")
    let fifthInsertedNode = new PriorityLinkedListNode("4", 4)
    list._insertNodeAtIndex(fifthInsertedNode, 3)
    Util.assertEquals(5, list.getSize(), "assert list size is as expected")
    let sixthInsertedNode = new PriorityLinkedListNode("6", 6)
    list._insertNodeAtIndex(sixthInsertedNode, 5)
    Util.assertEquals(6, list.getSize(), "assert list size is as expected")
    Util.assertEquals(-1, list._getIndexOfLabel("0"), "assert label is found where expected")
    Util.assertEquals(0, list._getIndexOfLabel("1"), "assert label is found where expected")
    Util.assertEquals(1, list._getIndexOfLabel("2"), "assert label is found where expected")
    Util.assertEquals(2, list._getIndexOfLabel("3"), "assert label is found where expected")
    Util.assertEquals(3, list._getIndexOfLabel("4"), "assert label is found where expected")
    Util.assertEquals(4, list._getIndexOfLabel("5"), "assert label is found where expected")
    Util.assertEquals(5, list._getIndexOfLabel("6"), "assert label is found where expected")
    Util.assertEquals(-1, list._getIndexOfLabel("7"), "assert label is found where expected")
    Util.assertEquals(6, list.getSize(), "check list size after inserting all nodes")
    Util.assertEquals(0, list._getIndexOfPriority(0), "check index of priority on full list")
    Util.assertEquals(0, list._getIndexOfPriority(1), "check index of priority on full list")
    Util.assertEquals(1, list._getIndexOfPriority(2), "check index of priority on full list")
    Util.assertEquals(2, list._getIndexOfPriority(3), "check index of priority on full list")
    Util.assertEquals(3, list._getIndexOfPriority(4), "check index of priority on full list")
    Util.assertEquals(4, list._getIndexOfPriority(5), "check index of priority on full list")
    Util.assertEquals(5, list._getIndexOfPriority(6), "check index of priority on full list")
    Util.assertEquals(6, list._getIndexOfPriority(7), "check index of priority on full list")
}

function _testInsertNodeWithPriority() {
    // insert items and check list size as we go
    let list = new PriorityLinkedList()
    Util.assertEquals(0, list.getSize(), "assert list size is as expected")
    Util.assertEquals(0, list._getIndexOfPriority(3), "check index of priority of first element that we are about to insert")
    let firstInsertedNode = new PriorityLinkedListNode("3", 3)
    ...
    Util.assertEquals("1", current.label, "assert head label is right")
    Util.assertEquals(null, current.previous, "assert head.previous is right")
    Util.assertEquals("2", current.next.label, "assert head.next is right")
    // check element 2
    current = current.next
    Util.assertEquals("2", current.label, "assert label is right")
    Util.assertEquals("1", current.previous.label, "assert .previous is right")
    Util.assertEquals("3", current.next.label, "assert .next is right")
    // check element 3
    current = current.next
    Util.assertEquals("3", current.label, "assert label is right")
    Util.assertEquals("2", current.previous.label, "assert .previous is right")
    Util.assertEquals("4", current.next.label, "assert .next is right")
    // check element 4
    current = current.next
    Util.assertEquals("4", current.label, "assert label is right")
    Util.assertEquals("3", current.previous.label, "assert .previous is right")
    Util.assertEquals("5", current.next.label, "assert .next is right")
    // check element 5
    current = current.next
    Util.assertEquals("5", current.label, "assert label is right")
    Util.assertEquals("4", current.previous.label, "assert .previous is right")
    Util.assertEquals("6", current.next.label, "assert .next is right")
    // check element 6
    current = current.next
    Util.assertEquals("6", current.label, "assert label is right")
    Util.assertEquals("5", current.previous.label, "assert .previous is right")
    Util.assertEquals(null, current.next, "assert .next is right")
...
function _testRemoveNode() {
    ...
    Util.assertEquals(5, list.getSize(), "assert list size is as expected")
    Util.assertEquals(0, list._getIndexOfLabel("2"), "assert label is found where expected")
    Util.assertEquals(1, list._getIndexOfLabel("3"), "assert label is found where expected")
    Util.assertEquals(2, list._getIndexOfLabel("4"), "assert label is found where expected")
    Util.assertEquals(3, list._getIndexOfLabel("5"), "assert label is found where expected")
    Util.assertEquals(4, list._getIndexOfLabel("6"), "assert label is found where expected")
    // remove last element
    list.removeNode("6")
    ...
    Util.assertEquals(3, list.getSize(), "assert list size is as expected")
    Util.assertEquals(0, list._getIndexOfLabel("2"), "assert label is found where expected")
    Util.assertEquals(1, list._getIndexOfLabel("4"), "assert label is found where expected")
    Util.assertEquals(2, list._getIndexOfLabel("5"), "assert label is found where expected")
    ...
    Util.assertEquals(4, list.getSize(), "assert list size is as expected")
    Util.assertEquals(0, list._getIndexOfLabel("2"), "assert label is found where expected")
    Util.assertEquals(1, list._getIndexOfLabel("5"), "assert label is found where expected")
    Util.assertEquals(2, list._getIndexOfLabel("4"), "assert label is found where expected")
    // remove second '4'
    list.removeNode("4")
